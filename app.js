...
function appendResult(){
    ...
    clearInterval(interval)
    quizHeader.innerHTML = "<h3>Result</h3>" /// Resutlt box Title
    ...
    divBody += "</thead></tbody>"
    divBody += `<td>${correctQuestions}/${questions.length}</td>` /// Score
    divBody += `<td>${(correctQuestions/questions.length)*100}%</td>`/// Percentage
    divBody += `<td>${formattedMinutes}:${formattedSeconds}</td>` /// Time
    divBody += "</tbody></table>"

    divBody += "<button class='btn btn-primary rstBtn' onclick='homePageReAttempt()'>Submit Scores</button>"
    quizBody.innerHTML = divBody
}
